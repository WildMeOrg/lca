...
class TrackingIdEmbeddings(Embeddings):
    """
    ...
    def from_embeddings(cls, 
                        ...
                        tracking_key='tracking_id',
                        multiplier=1.0):
        """Create TrackingIdEmbeddings from an existing Embeddings instance."""
        base_embeddings.print_func(f"Created TrackingIdEmbeddings with base {type(base_embeddings).__name__}")
        return cls(
    ...
    def _is_plausible(self, n0, n1):
        """Check if edge is plausible based on tracking id."""
        uuid1 = self.node2uuid[n0]
        uuid2 = self.node2uuid[n1]
        tracking_id1 = self.data_df.loc[self.data_df[self.id_key] == uuid1, self.tracking_key].squeeze()
        tracking_id2 = self.data_df.loc[self.data_df[self.id_key] == uuid2, self.tracking_key].squeeze()

        return tracking_id1 == tracking_id2

    def get_score(self, n0, n1):
        """Return 1.0 if same tracking id, base score if different."""
