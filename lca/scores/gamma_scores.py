...
class gamma_scores(object):  # NOQA
    ...
    def __init__(self, prior, gamma_pos, gamma_neg):
    ...
    def create_from_samples(cls, pos_samples, neg_samples):
        ...
        pos_dist = gamma.create_from_samples(pos_samples, is_positive=True)
        neg_dist = gamma.create_from_samples(neg_samples, is_positive=False)
    ...
    def get_pos_neg(self, score):
        ...
        score.
        """
        hp = self.gamma_pos.pdf(score)
        hn = self.gamma_neg.pdf(score)
        return hp, hn
    ...
    def random_score(self, is_match_correct):
        ...
        """
        if is_match_correct:
            s = self.gamma_pos.sample()
        else:
            s = self.gamma_neg.sample()
        return s
...
class gamma(object):
    def __init__(self, shape, loc, scale, is_positive):
        self.shape = shape
        self.loc = loc
        self.scale = scale
        self.is_positive = is_positive
        self.gamma_dist = stats.gamma(a=shape, scale=scale, loc=loc)
    ...
    def check_reverse(self, x):
        ...
        pointing forward
        """
        return x if self.is_positive else -x

    def pdf(self, x):
        return stats.gamma.pdf(self.check_reverse(x), self.shape, scale=self.scale, loc=self.loc)
    
    def cdf(self, x):
        return stats.gamma.cdf(self.check_reverse(x), self.shape, scale=self.scale, loc=self.loc)
    
    def sample(self):
        return self.check_reverse(stats.gamma.rvs(self.shape, scale=self.scale, loc=self.loc))
    
    def mean(self):
        return self.check_reverse(stats.gamma.mean(self.shape, scale=self.scale, loc=self.loc))
    
    def __repr__(self):
        return f"Gamma distribution(shape={self.shape}, loc={self.loc}, scale={self.scale}, is_positive={self.is_positive})"
    ...
    def create_from_samples(cls, samples, is_positive):
        ...
        return cls(shape, loc, scale, is_positive)